...
var contentWalker = new TreeWalker(null, SHOW_TEXT | SHOW_ELEMENT, function(
  node
) {
      ...
      if (nodeType === TEXT_NODE) {
        ...
        if (startsWithWS) {
          ...
          while ((sibling = walker.previousPONode())) {
            ...
            if (
              nodeName === 'IMG' ||
              (nodeName === '#text' && notWS.test(sibling.data))
            ) {
              break
            }
            if (!isInline(sibling)) {
...
proto._updatePath = function(range, force) {
  ...
  if (
    ...
    newPath =
      anchor && focus
        ? anchor === focus
          ? getPath(focus, this._root, this._config)
          : '(selection)'
        : ''
...
proto._saveRangeToBookmark = function(range) {
  ...
  if (
    startNode.compareDocumentPosition(endNode) & DOCUMENT_POSITION_PRECEDING
  ) {
...
proto.setHTML = function(html) {
  ...
  // Record undo state
  var range =
    this._getRangeAndRemoveBookmark() || this.createRange(root.firstChild, 0)
  this.saveUndoState(range)
...
var addPre = function(frag) {
  ...
  output.normalize()
  return fixCursor(
    this.createElement('PRE', this._config.tagAttributes.pre, [output]),
    root
  )
}
...
function removeFormatting(self, root, clean) {
  ...
  for (node = root.firstChild; node; node = next) {
    ...
    if (isInline(node)) {
      ...
      clean.appendChild(
        self.createDefaultBlock([
          removeFormatting(self, node, self._doc.createDocumentFragment())
        ])
      )
      continue
    }
